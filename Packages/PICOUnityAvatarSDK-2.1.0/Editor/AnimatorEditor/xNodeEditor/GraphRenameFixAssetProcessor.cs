...
namespace Pico.Avatar.XNodeEditor
{
    ...
    /// renaming a <see cref="Pico.Avatar.XNode.NodeGraph"/> asset, it appears that sometimes the v2 AssetDatabase will swap which asset
    /// is the main asset (present at top level) between the <see cref="Pico.Avatar.XNode.NodeGraph"/> and one of its <see cref="Pico.Avatar.XNode.Node"/>
    /// sub-assets. As a workaround until Unity fixes this, this asset processor checks all renamed assets and if it
    /// finds a case where a <see cref="Pico.Avatar.XNode.Node"/> has been made the main asset it will swap it back to being a sub-asset
    ...
    internal sealed class GraphRenameFixAssetProcessor : AssetPostprocessor
    {
            ...
            string[] movedFromAssetPaths)
        {
            for (int i = 0; i < movedAssets.Length; i++)
            {
                ...
                {
                    AssetDatabase.SetMainObject(nodeAsset.layer, movedAssets[i]);
