...
using UnityEditor;
using UnityEngine;
using Pico.Avatar.XNodeEditor.Internal;
...
namespace Pico.Avatar.XNodeEditor
{
    public partial class NodeEditorWindow
    {
        ...
        public static Pico.Avatar.XNode.Node[] copyBuffer = null;
        ...
        public Pico.Avatar.XNode.NodePort DraggedOutputPort { get { Pico.Avatar.XNode.NodePort result = draggedOutput; return result; } }
        /// <summary> Return the Hovered port or null if not exist </summary>
        public Pico.Avatar.XNode.NodePort HoveredPort { get { Pico.Avatar.XNode.NodePort result = hoveredPort; return result; } }
        /// <summary> Return the Hovered node or null if not exist </summary>
        public Pico.Avatar.XNode.Node HoveredNode { get { Pico.Avatar.XNode.Node result = hoveredNode; return result; } }

        private Pico.Avatar.XNode.Node hoveredNode = null;
        [NonSerialized] public Pico.Avatar.XNode.NodePort hoveredPort = null;
        [NonSerialized] private Pico.Avatar.XNode.NodePort draggedOutput = null;
        [NonSerialized] private Pico.Avatar.XNode.NodePort draggedOutputTarget = null;
        [NonSerialized] private Pico.Avatar.XNode.NodePort autoConnectOutput = null;
        ...
        public void Controls()
        {
            ...
            switch (e.type)
            {
                ...
                case EventType.MouseDrag:
                    if (e.button == 0)
                    {
                        ...
                        if (currentActivity == NodeActivity.DragNode)
                        {
                            ...
                            {
                                if (Selection.objects[i] is Pico.Avatar.XNode.Node)
                                {
                                    Pico.Avatar.XNode.Node node = Selection.objects[i] as Pico.Avatar.XNode.Node;
                                    ...
                                    {
                                        foreach (Pico.Avatar.XNode.NodePort output in node.Outputs)
                                        {
                                            Rect rect;
                                        ...
                                        foreach (Pico.Avatar.XNode.NodePort input in node.Inputs)
                                        {
                                            Rect rect;
                    ...
                    if (e.button == 0)
                    {
                        ...
                        if (IsHoveringPort)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Pico.Avatar.XNode.Node node = hoveredPort.node;
                                    Pico.Avatar.XNode.NodePort output = hoveredPort.Connection;
                ...
                case EventType.MouseUp:
                    if (e.button == 0)
                    {
                        ...
                        if (IsDraggingPort)
                        {
                            ...
                            {
                                Pico.Avatar.XNode.Node node = draggedOutputTarget.node;
                        ...
                        {
                            IEnumerable<Pico.Avatar.XNode.Node> nodes = Selection.objects.Where(x => x is Pico.Avatar.XNode.Node).Select(x => x as Pico.Avatar.XNode.Node);
                            foreach (Pico.Avatar.XNode.Node node in nodes) EditorUtility.SetDirty(node);
                            if (NodeEditorPreferences.GetSettings().autoSave && !Application.isPlaying) AssetDatabase.SaveAssets();
                        }
                        else if (!IsHoveringNode)
                        {
                ...
                case EventType.KeyDown:
                    ...
                    {
                        if (Selection.objects.Any(x => graph.nodes.Contains(x as Pico.Avatar.XNode.Node)))
                        {
                            foreach (Pico.Avatar.XNode.Node node in graph.nodes)
                            {
                                DeselectNode(node);
                            }
                        }
        ...
        public void Home()
        {
            var nodes = Selection.objects.Where(o => o is Pico.Avatar.XNode.Node).Cast<Pico.Avatar.XNode.Node>().ToList();
            if (nodes.Count > 0)
            {
        ...
        public void RemoveSelectedNodes()
        {
            ...
            {
                if (item is Pico.Avatar.XNode.Node)
                {
                    Pico.Avatar.XNode.Node node = item as Pico.Avatar.XNode.Node;
        ...
        public void MoveNodeToTop(Pico.Avatar.XNode.Node node)
        {
            int index;
        ...
        public void DuplicateSelectedNodes()
        {
            ...
            Pico.Avatar.XNode.Node[] selectedNodes = Selection.objects.Select(x => x as Pico.Avatar.XNode.Node).Where(x => x != null && x.layer == graph).ToArray();
        ...
        {
            copyBuffer = Selection.objects.Select(x => x as Pico.Avatar.XNode.Node).Where(x => x != null && x.layer == graph).ToArray();
        }
        ...
        private void InsertDuplicateNodes(Pico.Avatar.XNode.Node[] nodes, Vector2 topLeft)
        {
            ...
            Dictionary<Pico.Avatar.XNode.Node, Pico.Avatar.XNode.Node> substitutes = new Dictionary<Pico.Avatar.XNode.Node, Pico.Avatar.XNode.Node>();
            for (int i = 0; i < nodes.Length; i++)
            {
                Pico.Avatar.XNode.Node srcNode = nodes[i];
                ...
                Pico.Avatar.XNode.Node newNode = graphEditor.CopyNode(srcNode);
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                Pico.Avatar.XNode.Node srcNode = nodes[i];
                if (srcNode == null) continue;
                foreach (Pico.Avatar.XNode.NodePort port in srcNode.Ports)
                {
                    for (int c = 0; c < port.ConnectionCount; c++)
                    {
                        Pico.Avatar.XNode.NodePort inputPort = port.direction == Pico.Avatar.XNode.NodePort.IO.Input ? port : port.GetConnection(c);
                        Pico.Avatar.XNode.NodePort outputPort = port.direction == Pico.Avatar.XNode.NodePort.IO.Output ? port : port.GetConnection(c);

                        Pico.Avatar.XNode.Node newNodeIn, newNodeOut;
        ...
        bool IsHoveringTitle(Pico.Avatar.XNode.Node node)
        {
        ...
        public void AutoConnect(Pico.Avatar.XNode.Node node)
        {
            if (autoConnectOutput == null) return;

            // Find compatible input port
            Pico.Avatar.XNode.NodePort inputPort = node.Ports.FirstOrDefault(x => x.IsInput && graphEditor.CanConnect(autoConnectOutput, x));
