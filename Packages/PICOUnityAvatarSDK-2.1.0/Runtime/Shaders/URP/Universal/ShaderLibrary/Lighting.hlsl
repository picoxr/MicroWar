...
half4 UniversalFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half3 emission, half alpha, half occlusion
#ifdef PAV_ToonShadowMap
, half4 toonShadowAlbedo
#endif
)
{
#ifdef PAV_HAS_ADDITIVE_GI
    ...
    // only direct diffuse part.
    half3 finalColor = diffuse * inputData.bakedGI * occlusion;
    ...
    half NdotL = saturate(dot(inputData.normalWS, mainLight.direction));
    //..
    half toonShadow = (NdotL + inputData.bakedGI) * occlusion;

    #if defined(_SCREEN_SPACE_OCCLUSION)
        AmbientOcclusionFactor aoFactor = GetScreenSpaceAmbientOcclusion(inputData.normalizedScreenSpaceUV);
        toonShadow *= aoFactor.directAmbientOcclusion;
        occlusion = min(surfaceData.occlusion, aoFactor.indirectAmbientOcclusion);
    #endif

    //.. if in shadow side, use shadoe albedo.
    #ifdef PAV_ToonShadowMap
    if(toonShadow < 0.3){ // cut off color.
        diffuse = toonShadowAlbedo;
    }
    #endif
    ...
    half3 finalColor = diffuse.rgb * (inputData.bakedGI + radiance) * occlusion;
    ...
    if(toonShadow > 0.5)
    {
        ...
        if(pow(NdotH, 10) > 0.6)
        {
            finalColor += diffuse * specularGloss.rgb * lightAttenuation;// * surfaceData.albedo.rgb * 0.5;
        }
    }
    #endif
    ...
    #ifdef PAV_HAS_SPECULAR
    //#if defined(_SPECGLOSSMAP) || defined(_SPECULAR_SETUP)
...
// simplest fragmet.
half4 UniversalFragmentSimplest(InputData inputData, half4 diffuse)
{
#ifdef PAV_HAS_ADDITIVE_GI
    // add extra gi for some dark scene.
    inputData.bakedGI += _AdditiveGI;
    ...
    half3 diffuseColor = inputData.bakedGI + LightingLambert(mainLight.color, mainLight.direction, inputData.normalWS);
    
    half3 finalColor = diffuseColor * diffuse.rgb;
