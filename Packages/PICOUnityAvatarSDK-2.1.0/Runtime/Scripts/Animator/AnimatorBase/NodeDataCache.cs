...
namespace Pico.Avatar.XNode
{
	...
	public static class NodeDataCache
	{
		...
		public static List<FieldInfo> GetNodeFields(System.Type nodeType)
		{
			...
			while ((tempType = tempType.BaseType) != typeof(Pico.Avatar.XNode.Node))
			{
		...
		private static void CachePorts(System.Type nodeType)
		{
			...
			for (int i = 0; i < fieldInfo.Count; i++)
			{
				...
				UnityEngine.Serialization.FormerlySerializedAsAttribute formerlySerializedAsAttribute =
					attribs.FirstOrDefault(x => x is UnityEngine.Serialization.FormerlySerializedAsAttribute) as
