...
namespace Pico.Avatar.XNodeEditor
{
    ...
    public partial class NodeEditorWindow
    {
        ...
        private List<Pico.Avatar.XNode.Node> culledNodes;
        ...
        void ShowPortContextMenu(Pico.Avatar.XNode.NodePort hoveredPort)
        {
            GenericMenu contextMenu = new GenericMenu();
            ...
            if (NodeEditorPreferences.GetSettings().createFilter)
            {
                ...
                if (hoveredPort.direction == Pico.Avatar.XNode.NodePort.IO.Input)
                    graphEditor.AddContextMenuItems(contextMenu, hoveredPort.ValueType, Pico.Avatar.XNode.NodePort.IO.Output);
                else
                    graphEditor.AddContextMenuItems(contextMenu, hoveredPort.ValueType, Pico.Avatar.XNode.NodePort.IO.Input);
            }
            contextMenu.DropDown(new Rect(Event.current.mousePosition, Vector2.zero));
            if (NodeEditorPreferences.GetSettings().autoSave && !Application.isPlaying) AssetDatabase.SaveAssets();
        }
        ...
        public void DrawConnections()
        {
            ...
            foreach (Pico.Avatar.XNode.Node node in graph.nodes)
            {
                ...
                foreach (Pico.Avatar.XNode.NodePort output in node.Outputs)
                {
                    ...
                    {
                        Pico.Avatar.XNode.NodePort input = output.GetConnection(k);
        ...
        private void DrawNodes()
        {
            ...
            if (Selection.activeObject != null && Selection.activeObject is Pico.Avatar.XNode.Node)
            {
            ...
            List<Pico.Avatar.XNode.NodePort> removeEntries = new List<Pico.Avatar.XNode.NodePort>();

            if (e.type == EventType.Layout) culledNodes = new List<Pico.Avatar.XNode.Node>();
            for (int n = 0; n < graph.nodes.Count; n++)
            {
                // Skip null nodes. The user could be in the process of renaming scripts, so removing them at this point is not advisable.
                if (graph.nodes[n] == null) continue;
                if (n >= graph.nodes.Count) return;
                Pico.Avatar.XNode.Node node = graph.nodes[n];
                ...
                //if playing current state and Application.playing  is true, set this node highlight.
                bool isPlayingState = (node is AvatarAnimationStateNode ? (node as AvatarAnimationStateNode).isPlayingState : false) && Application.isPlaying;
                if (isPlayingState)
                {
                    GUIStyle highlightStyle = new GUIStyle(nodeEditor.GetBodyHighlightStyle());
                    GUI.color = NodeEditorPreferences.GetSettings().playingStateColor;
                ...
                if (isPlayingState) GUILayout.EndVertical();
        ...
        private bool ShouldBeCulled(Pico.Avatar.XNode.Node node)
        {
