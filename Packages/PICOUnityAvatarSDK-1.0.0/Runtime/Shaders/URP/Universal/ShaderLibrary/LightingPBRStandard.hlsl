#ifndef UNIVERSAL_LIGHTING_PBRSTANDARD_INCLUDED
#define UNIVERSAL_LIGHTING_PBRSTANDARD_INCLUDED
...
float ThickFilmReflectance(float cos0, float lambda, float thick, float n0, float n1, float n2) {
    ...
    float beamRatio = (n2 * cos2) / (n0 * cos0 + 0.001);
...
half4 UniversalFragmentPBR(InputData inputData, SurfaceData surfaceData)
{
    ...
    // temporary shadow for avatar editor.
#ifdef  PAV_FADE_GROUND_SHADOW
    float shadowRange = saturate(length(inputData.positionWS.xyz - _CenterPosition.xyz) * (1.0 / _ShadowFadeRange));
    mainLight.shadowAttenuation = lerp(mainLight.shadowAttenuation, 1, shadowRange);
#endif
