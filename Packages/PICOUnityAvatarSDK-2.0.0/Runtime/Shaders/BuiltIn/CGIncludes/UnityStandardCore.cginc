...
inline FragmentCommonData SpecularSetup (float4 i_tex)
{
    ...
    half3 albedo = ApplyAlbedo(PAV_SAMPLE_ALBEDO_ALPHA(i_tex.xy).rgb, baseColor, shaderType);
...
{
    half4 albedoAlpha = PAV_SAMPLE_ALBEDO_ALPHA(i_tex.xy);
    half4 albedoMaskAlpha = PAV_SAMPLE_COLOR_REGIONS(i_tex.xy);
    half4 specGloss = SpecularGloss(i_tex.xy);
    half3 specColor = specGloss.rgb;
    half smoothness = specGloss.a;
    ...
    {  
        colorMask3.rgb = i_tex.xyz;
    }
    

    
    half oneMinusReflectivity;
    half3 albedo = ApplyAlbedo(albedoAlpha.rgb, baseColor.rgb, shaderType,albedoMaskAlpha, colorMask1, colorMask2, colorMask3, colorMask4, usingAlbedoHue);
    half3 diffColor = EnergyConservationBetweenDiffuseAndSpecular (albedo, specColor, /*out*/ oneMinusReflectivity);
...
VertexOutputForwardBase vertForwardBase (VertexInput v)
{
    ...
    PAV_GET_VERTEX_PN(v.vid, v.vertex, v.normal);
#else
    ...
    o.tex = TexCoords(v);
    float3 getMaterialIndex = float3(o.tex.xy, 0.0);
    PAV_GET_MATERIAL_INDEX(v.vid, getMaterialIndex);
    o.materialIndex = getMaterialIndex.z;
...
{
    PAV_FLIP_UV_Y(i.tex);
    PAV_GET_MATERIAL_DATA(i.materialIndex);
