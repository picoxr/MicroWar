        ...
        private void OnGUI()
        {
            ...
            if (input == null || input.Trim() == "")
            {
                if (GUILayout.Button("Revert to default") || (e.isKey && e.keyCode == KeyCode.Return))
                {
                    ...
                    NodeEditor.GetEditor((Pico.Avatar.XNode.Node)target, NodeEditorWindow.current).OnRename();
                    if (!string.IsNullOrEmpty(AssetDatabase.GetAssetPath(target)))
                    {
                        AssetDatabase.SetMainObject((target as Pico.Avatar.XNode.Node).layer.parentAnimator, AssetDatabase.GetAssetPath(target));
            ...
            else
            {
                if (GUILayout.Button("Apply") || (e.isKey && e.keyCode == KeyCode.Return))
                {
                    ...
                    if (target is AvatarAnimationStateNode)
                    {
                        //rename state name to target.name
                        (target as AvatarAnimationStateNode).stateName = input;
                    }
                }
            }
