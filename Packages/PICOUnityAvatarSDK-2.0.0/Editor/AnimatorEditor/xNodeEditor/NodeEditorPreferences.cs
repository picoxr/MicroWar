...
namespace Pico.Avatar.XNodeEditor
{
    ...
    public static class NodeEditorPreferences
    {
        ...
        private static Pico.Avatar.XNodeEditor.NodeGraphEditor lastEditor;
        ...
        public class Settings : ISerializationCallbackReceiver
        {
            ...
            public Color32 playingStateColor = new Color32(255, 255, 0, 255);
        ...
        {
            if (Pico.Avatar.XNodeEditor.NodeEditorWindow.current == null) return new Settings();

            if (lastEditor != Pico.Avatar.XNodeEditor.NodeEditorWindow.current.graphEditor)
            {
                object[] attribs = Pico.Avatar.XNodeEditor.NodeEditorWindow.current.graphEditor.GetType().GetCustomAttributes(typeof(Pico.Avatar.XNodeEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute), true);
                if (attribs.Length == 1)
                {
                    Pico.Avatar.XNodeEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute attrib = attribs[0] as Pico.Avatar.XNodeEditor.NodeGraphEditor.CustomNodeGraphEditorAttribute;
                    lastEditor = Pico.Avatar.XNodeEditor.NodeEditorWindow.current.graphEditor;
        ...
        public static SettingsProvider CreateXNodeSettingsProvider()
        {
            ...
            {
                guiHandler = (searchContext) => { Pico.Avatar.XNodeEditor.NodeEditorPreferences.PreferencesGUI(); },
                keywords = new HashSet<string>(new[] { "xNode", "node", "editor", nameof(Pico.Avatar.XNode.Node.layer), "connections", "noodles", "ports" })
            };
            return provider;
        }
#endif
        ...
        private static void NodeSettingsGUI(string key, Settings settings)
        {
            ...
            settings.playingStateColor = EditorGUILayout.ColorField("PlayingState", settings.playingStateColor);
