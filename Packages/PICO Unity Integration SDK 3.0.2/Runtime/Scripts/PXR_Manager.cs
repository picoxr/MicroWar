...
namespace Unity.XR.PXR
{
    public class PXR_Manager : MonoBehaviour
    {
        ...
        public void OnPostRenderCallBack(Camera cam)
        {
            ...
            if (openMRC && PXR_Plugin.System.UPxr_GetMRCEnable())
            {
                CreateMRCOverlay();
        ...
        private void UpdateMRCCam()
        {
            ...
            if (!PXR_Plugin.System.UPxr_GetMRCEnable())
            {
                if (mrcCamObjActived)
                {
                    ...
                    VstDisplayStatusChanged -= VstDisplayStatusChangedMethod;
                }
                return;
            }
        ...
        private void VstDisplayStatusChangedMethod(PxrVstStatus vstStatus)
        {
            PLog.i(TAG_MRC, $"VstDisplayStatusChangedMethod VstDisplayStatus = {vstStatus}, backgroundCamObj = {backgroundCamObj != null}");
            if (backgroundCamObj != null)
            {
                Camera camera = backgroundCamObj.GetComponent<Camera>();
                if (PxrVstStatus.Enabled == vstStatus) // MR
                {
                    camera.clearFlags = CameraClearFlags.SolidColor;
                    camera.backgroundColor = foregroundColor;
                }
                else if (PxrVstStatus.Disabled == vstStatus) // VR
                {
        ...
        private void InitMRCCam(Camera camera, bool isForeground)
        {
            ...
            camera.aspect = cameraInfo.width / cameraInfo.height;
