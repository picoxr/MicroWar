...
#region Bloom
        ...
        void SetupBloom(CommandBuffer cmd, RTHandle source, Material uberMaterial)
        {
            ...
            CoreUtils.SetKeyword(bloomMaterial, ShaderKeywordStrings.Bloom5taps, m_Bloom.GaussianTaps == BloomGaussianTaps._5Taps);
            ...
            if (mipCount <= 1)
            {
                RenderingUtils.ReAllocateIfNeeded(ref m_BloomMipUp[1], desc, FilterMode.Bilinear, TextureWrapMode.Clamp, name: m_BloomMipUp[1].name);
                RenderingUtils.ReAllocateIfNeeded(ref m_BloomMipDown[1], desc, FilterMode.Bilinear, TextureWrapMode.Clamp, name: m_BloomMipDown[1].name);

                Blitter.BlitCameraTexture(cmd, lastDown, m_BloomMipUp[1], RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, bloomMaterial, 1);
                Blitter.BlitCameraTexture(cmd, m_BloomMipUp[1], m_BloomMipDown[1], RenderBufferLoadAction.DontCare, RenderBufferStoreAction.Store, bloomMaterial, 2);
            }
            //Gaussian pyramid
            else
            {
            ...
            if (mipCount <= 1)
            {
                cmd.SetGlobalTexture(ShaderConstants._Bloom_Texture, m_BloomMipDown[1]);
            }
            else
            {
